...
export function getDB(env) {
  if (!env || !("DB" in env)) {
    ...
    throw new Error(`D1 binding named DB is missing. Available environment keys: ${available}`);
  }

  const db = env.DB;
  if (!db || typeof db.prepare !== "function") {
    const type = db === null ? "null" : typeof db;
    throw new Error(`Binding DB exists but is not a D1 database. Current type: ${type}`);
  }

  return db;
}

export function d1Error(error) {
  const message = error && error.message ? error.message : String(error);
  if (message.includes("RPC receiver") && message.includes("prepare")) {
    return new Error("Binding DB exists but is not connected as a D1 database in this Pages production deployment. Delete the current DB binding and add a D1 database binding named DB to the Production environment, then trigger a new GitHub commit deployment.");
  }
  return error;
}
...
export async function sendVerificationEmail(env, { to, code, fullName }) {
  // Verification emails are intentionally kept in English for a consistent official style.
  const siteName = env.SITE_NAME || "Jia Honours";
  const safeSiteName = escapeHtml(siteName);
  const safeFullName = escapeHtml(fullName);
  const subject = `${siteName} Verification Code`;
  const text = `Your Jia Honours verification code is ${code}. This code expires in 10 minutes.`;
  const html = `
    <div style="font-family:Arial,sans-serif;line-height:1.6;color:#1c1917">
      <h2>${safeSiteName}</h2>
      <p>${safeFullName ? `Dear ${safeFullName},` : "Hello,"}</p>
      <p>Your Jia Honours verification code is:</p>
